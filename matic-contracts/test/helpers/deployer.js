...
class Deployer {
  ...
  async deployCndlWETH() {
    const CndlWETH = await contracts.CndlWETH.new()
    await Promise.all([
      this.mapToken(CndlWETH.address, CndlWETH.address, false /* isERC721 */),
  ...
  async initializeChildChain(owner, options = { updateRegistry: true }) {
    ...
    if (!this.globalMatic) {
      // MRC20 comes as a genesis-contract at utils.ChildCndlTokenAddress
      if (process.env.SOLIDITY_COVERAGE) {
        utils.ChildCndlTokenAddress = (await contracts.MRC20.new()).address
        Object.freeze(utils.ChildCndlTokenAddress)
      }

      this.globalMatic = { childToken: await contracts.MRC20.at(utils.ChildCndlTokenAddress) }
    ...
    if (this.registry) {
      ...
      this.globalMatic.rootERC20 = await this.deployTestErc20({ mapToken: true, childTokenAdress: utils.ChildCndlTokenAddress })
    }
